...
%%
expname = 'experiments/itmv0_it4';
cachefile = 'cache/itmv0/iter4/params';
noitmtype = 'type5';
...
parfor i = 1:length(data)
    ...
    [conf3{i}] = reestimateObjectConfidences(res(i).spg, res(i).maxidx, data(i).x, iclusters, params);
...
for i = 1:length(names)
    ...
    h = ylabel('precision');
    set(h, 'fontsize', fontsize); 
    legend({['ours AP=' num2str(ap{i}, '%.03f')], ['no itm AP=' num2str(ap_gr{i}, '%.03f')], ['dets AP=' num2str(apbase{i}, '%.03f')]}, 'location', 'SouthWest');
    h = title(names{i});
    set(h, 'fontsize', fontsize); 
