if(0)
    ...
    dataroot = 'uiucdata'; 
    datadir = 'uiucdata'; % fullfile(dataroot, 'test');
    ...
    parfor i = 1:length(datafiles) % min(200, length(datafiles))
        ...
        if(~isempty(data(i).x))
            ...
            data(i).anno.scenetype = 0; % data(i).gpg.scenetype;
...
remove_idx = [];
for i = 1:length(data)
    if(isempty(data(i).x) || isempty(data(i).anno))
        remove_idx(end+1) = i;
    end
end
remove_idx
...
parfor i = 1:length(data)
    ...
    try
        params = paramsout;
        pg = parsegraph(1); % findConsistent3DObjects(data(i).gpg, data(i).x, data(i).iclusters);
