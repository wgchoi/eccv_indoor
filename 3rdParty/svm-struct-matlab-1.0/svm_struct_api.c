                    ...
                    STRUCT_LEARN_PARM *sparm)
{
  ...
  fn_array = mxGetField(sparm->mex, 0, "finalizeIterationFn") ;
  if (! fn_array) {
    mexErrMsgTxt("Field PARM.finalizeIterationFn not found") ;
  }
  model_array = newMxArrayEncapsulatingSmodel (sm) ;
  ...
  args[1] = (mxArray*) sparm->mex ; /* model (discard conts) */
  args[2] = model_array ;
  args[3] = mxCreateDoubleMatrix(1, 1, mxREAL);
  ptr = mxGetPr(args[3]);
  *ptr = ceps;
  ...
  destroyMxArrayEncapsulatingSmodel (model_array) ;
  destroyMxArrayEncapsulatingDoubleVector(args[3]);

  if (status) {
    mexErrMsgTxt("Error while executing PARM.LOSSFN") ;
  }
  if (! uIsRealScalar(out)) {
    mexErrMsgTxt("PARM.ABT must reutrn a scalar") ;
  }
